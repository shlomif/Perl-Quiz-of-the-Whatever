...
sub Graham
{
    ...
    if ($n == $last_n+1)
    {
        ...
        {
            next if (!defined($base[$j]));
            $base_vec_compose = $base_composition[$j];
            if (grep { $_ == $n } @$base_vec_compose)
            {
                $base[$j] = multiply_squaring_factors($base[$j], $n_sq_factors);
                $base_composition[$j] = [ grep { $_ != $n } @$base_vec_compose ];
            }
        }
        foreach my $p (@$n_sq_factors)
        {
            ...
            if (! (grep { $_ == $p } @{$base[$id]}))
            {
                $base[$id] = $base_composition[$id] = undef;
            }
        }
    }
    else
    ...
    for(my $i=$n+1;;$i++)
    {
        ...
        while (scalar(@$n_vec))
        {
            ...
            #printf("base[\$id]=" . join(",", @{$base[$id]}) . "\n");
            $n_vec = multiply_squaring_factors($n_vec, $base[$id]);
            #printf("\$n_vec=" . join(",", @{$n_vec}) . "\n");
